...
fn test_unicode_and_emoji_support() {
    ...
    let jp_path = layout.create_feature(jp_name, None).expect("Should create japanese branch");
    ...
    let emoji_path = layout.create_feature(emoji_name, None).expect("Should create emoji branch");
    ...
    for wt in &worktrees {
        println!(" - Name: '{}', Branch: '{}', Path: '{}'", wt.name, wt.branch, wt.path.to_string());
    }
    ...
    let found_jp = worktrees.iter().find(|wt| wt.name == expected_jp_slug || wt.branch == expected_jp_slug);
    assert!(found_jp.is_some(), "Should find Japanese worktree (as slug: {})", expected_jp_slug);
    
    let expected_emoji_slug = "fix-bug"; // Emoji is not alphanumeric, so it gets stripped/sanitized
    let found_emoji = worktrees.iter().find(|wt| wt.name == expected_emoji_slug || wt.branch == expected_emoji_slug);
    assert!(found_emoji.is_some(), "Should find Emoji worktree (sanitized as: {})", expected_emoji_slug);

    // 4. Remove
    layout.remove_feature(jp_name).expect("Should remove jp branch");
    assert!(!jp_path.exists(), "Path should be gone");
    
    layout.remove_feature(emoji_name).expect("Should remove emoji branch");
