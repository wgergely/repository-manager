...
pub fn windsurf_integration() -> GenericToolIntegration {
    GenericToolIntegration::new(ToolDefinition {
        meta: ToolMeta {
            name: "Windsurf".into(),
            slug: "windsurf".into(),
            description: Some("Windsurf AI IDE".into()),
        },
...
///
/// Prefer using `windsurf_integration()` factory function for new code.
pub type WindsurfIntegration = GenericToolIntegration;

/// Creates a new Windsurf integration (legacy API).
///
/// # Deprecated
...
mod tests {
    ...
    fn test_config_paths() {
        ...
        assert_eq!(paths, vec![".windsurfrules"]);
    }

    #[test]
    fn test_sync_creates_windsurfrules() {
        ...
        let content = fs::read_to_string(&windsurfrules_path).unwrap();
    ...
    fn test_sync_uses_managed_blocks() {
        ...
        let content = fs::read_to_string(temp_dir.path().join(".windsurfrules")).unwrap();
    ...
    fn test_sync_preserves_manual_content() {
        ...
        // Create windsurfrules with manual content
        let manual_content = "# Manual rules\n\nDo not modify managed blocks below.\n";
        fs::write(temp_dir.path().join(".windsurfrules"), manual_content).unwrap();
