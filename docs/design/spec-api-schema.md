...
This document explores a potential API schema for `agentic`, a command-line tool that could orchestrate configuration across multiple agentic coding tools (Claude Code, Cursor, Windsurf, Gemini Code Assist, GitHub Copilot). The concept is to provide a single source of truth for rules, skills, and settings while generating tool-specific configurations through a provider abstraction layer.
...
An orchestrator tool would likely require a set of commands for initializing project configuration, managing rules and skills as atomic entities, and synchronizing these settings across various agentic providers. The design should focus on provider-agnostic abstractions that can be translated into tool-specific requirements during the synchronization process.

---

## 2. Configuration Schema

The central configuration needs to handle project metadata, provider activation and paths, rule and skill management strategies, plugin installations, and fine-grained permission controls for AI tools. Using a human-readable format like TOML or YAML enables easy manual editing while supporting complex nested structures required for multi-provider orchestration.
...
```markdown
---
# .agentic/rules/coding-standards.md
id: coding-standards
...
A provider abstraction layer could enable translation between a central configuration and tool-specific formats. The following interfaces illustrate one approach to this pattern.
...
interface Provider {
  ...
  translateSettings(settings: AgenticSettings): Promise<ProviderOutput>;
...
class ClaudeProvider implements Provider {
  ...
  private generateClaudeMd(rules: Rule[]): string {
    ...
    content += '<!-- Generated by agentic - do not edit directly -->\n\n';
...
interface ProviderRegistry {
  ...
  getAll(): Provider[];
  getEnabled(config: AgenticConfig): Provider[];
...
version = "2.0.0"
description = "Security scanning integration for agentic configurations"
author = "Agentic Team <team@agentic.dev>"
license = "MIT"
repository = "https://github.com/agentic/security-scanner"

# Minimum agentic version required
...
interface Plugin {
  ...
  // Requirements
  agenticVersion: string;               // Semver range
...
interface RepositoryConfig {
  ...
  agenticPath: string;                  // Path to .agentic directory

  // Configuration
  config: AgenticConfig;                // Parsed config.toml
...
```text
project/
├── .agentic/                           # Agentic configuration root
...
```text
~/.agentic/                             # User-level configuration
...
│   ├── .claude -> ../.agentic/.claude  # Symlink to shared config
│   └── (source)
...
- Easier adoption - existing projects can add agentic alongside current configs
...
- Aligns with research findings from doc `03-worktree-patterns.md`
...
6. **Migration Tools**: Convert existing tool configs to agentic format
...
| :-------------------- | :------------------------------ | :----------------------------- |
| `AGENTIC_CONFIG_PATH` | Override config file location   | `.agentic/config.toml`         |
| `AGENTIC_GLOBAL_PATH` | Override global config location | `~/.agentic`                   |
| `AGENTIC_REGISTRY`    | Override plugin registry URL    | `https://registry.agentic.dev` |
| `AGENTIC_NO_COLOR`    | Disable colored output          | `false`                        |
| `AGENTIC_VERBOSE`     | Enable verbose logging          | `false`                        |
| `AGENTIC_DRY_RUN`     | Global dry-run mode             | `false`                        |
