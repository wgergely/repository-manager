...
- [x] Pre/post worktree creation hooks (install deps, copy configs)
- [x] Pre/post hook execution (run tests, lint, validate)
- [x] Hook configuration in `.repository/hooks/`
- **Source**: [05-feature-gaps-opportunities.md](audits/2026-02-17-deep-audit/05-feature-gaps-opportunities.md) S1.2
- **Closed**: 2026-02-24 — `run_hooks` with `PreBranchCreate`/`PostBranchCreate`/`PreBranchDelete`/`PostBranchDelete` in `crates/repo-cli/src/commands/branch.rs`

### 3.2 Config Governance
- [x] `repo rules lint` - validate consistency across tools
- [x] `repo rules diff` - show config drift
- [x] `repo rules export --format agents` - AGENTS.md export
- [x] `repo rules import AGENTS.md` - AGENTS.md import
...
- **Closed**: 2026-02-24 — `crates/repo-cli/src/commands/governance.rs` (272 lines); ledger audit trail remains unimplemented

### 3.3 `repo open` Command
- [x] `repo open <worktree> [--tool cursor|vscode|zed]`
- [x] Launch IDE/editor in specified worktree with configs applied
- **Source**: [05-feature-gaps-opportunities.md](audits/2026-02-17-deep-audit/05-feature-gaps-opportunities.md) S1.2
- **Closed**: 2026-02-24 — `crates/repo-cli/src/commands/open.rs` (263 lines)

### 3.4 MCP Server Completion
- [x] Implement git push/pull/merge MCP primitives
...
- **Closed**: 2026-02-24 — `handle_git_push`/`handle_git_pull`/`handle_git_merge` in `crates/repo-mcp/src/handlers.rs`; health checks and discovery remain unimplemented

### 3.5 Global Config Layer
- [x] `~/.config/repo/` for user-level defaults
- [ ] Org-level config layer (git-hosted shared config)
- [x] Config hierarchy: global -> org -> project -> worktree
- **Source**: [02-technical-audit.md](audits/2026-02-17-deep-audit/02-technical-audit.md) S6
- **Closed**: 2026-02-24 — `ConfigResolver` with global/local config layering in `crates/repo-core/src/config/resolver.rs`; org-level config layer remains unimplemented
...
*Updated: 2026-02-24 -- Phase 3 core items confirmed implemented and closed.*
